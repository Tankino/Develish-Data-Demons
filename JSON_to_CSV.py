        ...
        with open(f'../People/{alphabet[i]}_people.json', encoding = 'utf-8') as json_file:
            ...
            for person in people:
                if 'fiction' in str(person['http://www.w3.org/1999/02/22-rdf-syntax-ns#type_label']):
                    ...
                    religion = clean_data(person, 'religion')
                    nationality = clean_data(person, 'nationality')
                    country = clean_data(person, 'country')
                    region = clean_data(person, 'region')
                    print(f'{title}, {gender}, {creator}, {person.get("http://purl.org/dc/elements/1.1/description")}')
                    #csv_file.write(f'{title}, {gender}, {creator}, {genre}, {firstappearance}, {species}, {birthdate}, {birthyear}, {deathdate}, {deathyear}, {deathcause}, {ethnicity}, {religion}, {nationality}, {country}, {region}\n')
